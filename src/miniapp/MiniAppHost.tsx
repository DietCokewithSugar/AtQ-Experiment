...
import { globalColorForUser } from "@/collab/user-colors";
...
export function MiniAppHost({
  ...
  className?: string;
  appCode?: string;
  onMouseMove?: (x: number, y: number, viewport: Viewport) => void;
}) {
  const { docRef, docId, docLoading } = useDocumentContext();
  const appStateRef = child(docRef, "miniAppState");
        ...
        try {
          ...
          switch (data.type) {
            ...
            case "aiGenerateImage": {
              let { requestId } = data || {};
              try {
                let generatedImageUrl = await generateImage(ai, {
                  prompt: data.prompt,
                });
